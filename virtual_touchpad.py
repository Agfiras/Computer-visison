...
def compute_homography(touchpad): # Compute the homography matrix
    rect = np.array([[0, 0], [300, 0], [300, 200], [0, 200]], dtype="float32") # Define the rectangle
    h, _ = cv2.findHomography(touchpad, rect) # Compute the homography matrix
    return h
...
def detect_finger(mask): # Detect the finger in the segmented skin
    ...
    max_defect = max(defects, key=lambda x: x[0][3]) # Find the defect with the largest depth
    s, e, f, d = max_defect[0] # Get the start, end, farthest point, and depth of the defect
    farthest_point = tuple(largest_contour[f][0]) # Get the farthest point
